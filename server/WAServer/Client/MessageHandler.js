...
} = require("../../app/config/commands.js");
...
class Message {
    ...
    async messageHandler(m, ilsya) {
        ...
        logger("info", `[SESSION] body WEBHOOK ${body}`);
        if (getwebook) {
        ...
        // command
        try {
            let cmd = Array.from(commands.values()).find((v) =>
                v.cmd.find((x) => x.toLowerCase() == commandbody.toLowerCase())
            );
            if (cmd) {
